...
main() {
    ...
    openssl req -x509 -new -sha512 -noenc -key "${certs_dir}/ca.key" -days 3653 -config "./ca-controllers.conf" -out "${certs_dir}/ca.crt"
    ...
    for cid in ${control_plane_certs[@]}; do
        gen_cert $certs_dir "ca-controllers.conf" $cid
    done

    ## Generate certificates for nodes
    for cid in ${nodes[@]}; do
        gen_cert $certs_dir "ca-nodes.conf" $cid
    done
}
