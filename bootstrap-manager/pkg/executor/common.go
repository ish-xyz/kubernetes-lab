...
func (e *Executor) ListConfigMaps(desiredNumber, maxRetries, interval int) (*corev1.ConfigMapList, error) {
	...
	for retry := 0; retry < maxRetries; retry++ {
		...
		// NOTE: this check is critical to the main workflow, don't remove
		// combined with early failures when systemd services are not running,
		// this checks prevents, other controllers nodes from being migrated
		// if one or more and in a degraded state.
